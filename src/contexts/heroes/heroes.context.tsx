...
export const HeroesProvider = ({ children }: Props) => {
  ...
  const filteredHeroes = useMemo(() => heroes.filter((hero) => hero.name.toLowerCase().includes(searchString.toLowerCase()))
    , [heroes, searchString])

  return <HeroesContext.Provider value={{
    heroes,
    filteredHeroes,
    setSearch,
    search: searchString,
  }}>{children}</HeroesContext.Provider>;
};
